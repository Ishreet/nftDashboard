...
class projectsBoard {
	...
	updateList({ listId, update }) {
		return new Promise((resolve, reject) => {
			try {
				...
				const list = clonedProject.lists.find(
					(_list) => _list.id === listId
				);
	...
	updateTask({ taskId, update }) {
		return new Promise((resolve, reject) => {
			try {
				...
				const task = clonedProject.tasks.find(
	...
	moveTask({ taskId, position, listId }) {
		return new Promise((resolve, reject) => {
			try {
				const clonedProject = copy(project);
				...
				const sourceList = clonedProject.lists.find(
				...
				sourceList.taskIds = sourceList.taskIds.filter(
					(_taskId) => _taskId !== taskId
				);

				if (listId) {
					const destinationList = clonedProject.lists.find(
						(list) => list.id === listId
					);
					...
					destinationList.taskIds.splice(
						position,
						0,
						task.id
					);
