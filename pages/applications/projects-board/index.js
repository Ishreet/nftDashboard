...
function ApplicationsProjectsBoard() {
	...
	const tabs = [
		{
			value: 'react_migration',
			label: t('React Project Migration')
		},
		{
			value: 'development_meeting',
			label: t('Development Meeting')
		},
		{
			value: 'marketing_campaign',
			label: t('Marketing Campaign')
		}
	];
	...
	const handleDragEnd = async ({
		...
		try {
			...
			if (source.droppableId === destination.droppableId) {
				await dispatch(
					moveTask(draggableId, destination.index)
				);
			} else {
				await dispatch(
					moveTask(
			...
			console.error(err);
			enqueueSnackbar(
				t('There was an error, try again later'),
				{
					variant: 'error',
					anchorOrigin: {
						vertical: 'bottom',
